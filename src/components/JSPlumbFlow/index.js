...
export default class Index extends Component {
  ...
  constructor(props) {
    ...
    /**
     * 在构造函数的时候接收传递过来的props getInstance,handleCancel handleOk
  ...
  createNewNode(visoData){
    ...
    if(type === proConfig.nodeFlowType.userTask) {
      ...
      newInfo.candidateUsersArray = visoData.createInfo.candidateUsersArray;
      newInfo.candidateGroupsArray = visoData.createInfo.candidateGroupsArray;
      newInfo.assignee = visoData.createInfo.assignee;
      newInfo.assigneeArray = visoData.createInfo.assigneeArray;
      newInfo.assigneeArrayOther = visoData.createInfo.assigneeArrayOther;
      newInfo.completionCondition = visoData.createInfo.completionCondition;
      newInfo.completionConditionType = visoData.createInfo.completionConditionType;
      newInfo.completionConditionArray = visoData.createInfo.completionConditionArray;
      newInfo.formKey = visoData.createInfo.formKey;
      newInfo.isSequential= visoData.createInfo.isSequential;;
      newInfo.assigneeGroup = visoData.createInfo.assigneeGroup;
      newInfo.assigneeGroupArray = visoData.createInfo.assigneeGroupArray;
      newInfo.assigneeGroupArrayOther = visoData.createInfo.assigneeGroupArrayOther;
    }
    /**
     * ServiceTask需要有配置规则serviceTaskRule字段  assigneeGroup    assigneeArrayGroup
     */
  ...
  bindConnectionAddLabel() {
    ...
    this.jsPlumbForword.bind('beforeDrop',  (info) => {
      // console.log('11111111111111111', info);
     ...
     // if( sourceIdData[0] === "exclusive"  || targetIdData[0] === "exclusive" || sourceIdData[0] === "inclusive" || targetIdData[0] === "inclusive" ) {
      if(sourceIdData[0] === "exclusive" || sourceIdData[0] === "inclusive") {
        CurrentLineInfo = {
  ...
  saveDataHandle(){
    ...
    //    if(nodeData[i].type==='userTask'){
    //      nodeData[i].isSequential=nodeData[i].isSequential==='true'?true:(nodeData[i].isSequential==='false'?false:null)
    //    }
    // }
