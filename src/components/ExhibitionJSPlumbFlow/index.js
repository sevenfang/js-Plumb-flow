...
export default class Index extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      rulesPositionY:0,
      remark:'很多事情是没有原因的',
      currentElementInfos:{},//当前代办节点的信息
      elementInfos:{},//要驳回到的节点
      ensureRollBack:false,//确定是否流程回退的弹框
      processRejection:{},//流程驳回需要的数据
      labelData:[],
      source:{},
      target:{},
      expressionVisible:-1,
    ...
    this.ensureRollBackOK=this.ensureRollBackOK.bind(this);
    this.ensureRollBackCancel=this.ensureRollBackCancel.bind(this);
    this.rollBackReason=this.rollBackReason.bind(this);
 ...
 componentWillReceiveProps(nextProps){
  ...
  getAnchorID(anchorInfo, info) {
    ...
    // console.log('fdsafdsfad/////////////',finalUidConnect)
  ...
  getLabelSetInfo(labelText, sourceId, targetId, index) {
    ...
    const labelInfos =
     `<div style="position:relative" id='${sourceId}-${targetId}'>
       <div>${labelText}</div>
       <!--<div style="position:absolute;top:20px;left:0;display: ">{labelTextRules}</div>-->
     </div>`;
    ...
    return {
      ...
      events: {
        mouseover: (event) => {
          let labelDom = document.getElementById(`${sourceId}-${targetId}`);
          let boxContent = document.getElementById('boxContent');
          let pos=this.getPosition(labelDom);
          let boxPos=this.getPosition(boxContent);
       
         //  console.log(pos.top-boxPos.top-boxContent.scrollTop)
         //  console.log(pos.left-boxPos.left-boxContent.scrollLeft)
         // console.log(pos.top-boxPos.top)
         // console.log(pos.left-boxPos.left)
         // console.log(labelData)
         // console.log(labelData.rules)
         // console.log(this.reCreatorData.connectionData)
         if(labelData.rules!==''){
            this.setState({
              expressionVisible:labelData.rules,
              source:labelData.source,
              target:labelData.target,
              labelData:labelData,
              rulesPositionX:pos.left-boxPos.left,
              rulesPositionY:pos.top-boxPos.top,
              ruleModalWidth:labelData.rules.length>300?300:150,
            });
          }
        },

       mouseout: (labelOverlay) => {
        this.setState({
          expressionVisible:-1,
        });
       }
  ...
  loadDataAndPaint() {
    ...
    /**
     * 定义需要绑定的事件对象
     * @type {{onClick: *}} currentElementInfos
     */
    const eventObj = {
      onClick: (e,info)=>{
       // console.log(e,info,this.state.dataSource)
       const { requestUrl, requestUrlHashKey,id,sign } = this.props;
       if(sign===undefined) return false;
       if(e.type==='startEvent'||e.type==='userTask'){
         let elementInfos = {};
         let currentElementInfos = {};
         for(let i=0;i<this.state.dataSource.elementInfos.length;i++){
           if(e.id===this.state.dataSource.elementInfos[i].elementId){
             elementInfos = this.state.dataSource.elementInfos[i];
           }
           if(this.state.dataSource.currentElementId===this.state.dataSource.elementInfos[i].elementId){
              currentElementInfos = this.state.dataSource.elementInfos[i];
           }
         }
       if(this.state.dataSource.currentElementId!==elementInfos.elementId&&elementInfos.elementId){
         this.setState({
           elementInfos,
           currentElementInfos,
           ensureRollBack:true,//确定是否流程回退的弹框
         })
        }
       }
      },
   ...
   // console.log(JSON.parse(this.state.dataSource.bpmnInfo.designJson))
    this.reCreatorData = new VirtualDOMExhibition(this.state.dataSource, eventObj);
    let nodeList = this.reCreatorData.init(); //所有的节点信息
    // console.log(nodeList)
    // console.log(this.state.dataSource.showElementIds)
    //遍历循环nodeList，确定哪些节点显示，哪些节点不显示
    let needRenderNodeList = [];//需要显示的节点信息
    let needRenderConnectionArr = [];//需要显示的连线的信息
    if(this.state.dataSource.showElementIds){
      for (let j = 0; j < this.state.dataSource.showElementIds.length; j++) {
        for (let i = 0; i < nodeList.length; i++) {
          if (nodeList[i].key === this.state.dataSource.showElementIds[j]) {
            needRenderNodeList.push(nodeList[i])
          }
        }
      }
      for (let k = 0; k < this.reCreatorData.connectionData.length; k++) {
        for (let j = 0; j < this.state.dataSource.showElementIds.length; j++) {
          if (this.reCreatorData.connectionData[k].id === this.state.dataSource.showElementIds[j]) {
            needRenderConnectionArr.push(this.reCreatorData.connectionData[k])
          }
        }
      }
    }
   console.log('this.reCreatorData.nodeData',needRenderNodeList)
   console.log('this.reCreatorData.nodeData',needRenderConnectionArr)
   console.log('this.reCreatorData.nodeData',this.reCreatorData.connectionData)
   console.log('this.reCreatorData.nodeData',this.props)
    this.setState(
        () => ({nodeList,needRenderNodeList}),
        () => {
          (this.props.sign&&this.props.sign==='claimTasks'?needRenderConnectionArr:this.reCreatorData.connectionData).forEach((info, index) => {
  ...
  ensureRollBackOK(){
     const { requestUrl, requestUrlHashKey,id,sign } = this.props;
     let currentTaskId =  this.state.currentElementInfos.taskId;
     let revokeTaskId = this.state.elementInfos.elementId.substring(0,5)==='start'?null:this.state.elementInfos.taskId;
     axios.post(`${requestUrl.substring(0,26)}bank/process/rollbackProcess`,({
       currentTaskId:id,
 ...
 // DOM渲染
  render() {
    const { containerIdPreFix } = this.props;
    // console.log('////////////////',this.state.expressionVisible)
    // console.log('////////////////',this.state.rulesPositionX)
    // console.log('////////////////',this.state.rulesPositionY)
    return (
        <div id='boxContent' style={{ position:'relative',width: "100%", overflow: "scroll"}}>
          <Modal
            title="流程回退"
            visible={this.state.ensureRollBack}
            onOk={this.ensureRollBackOK}
            onCancel={this.ensureRollBackCancel}
            okText='确定'
            cancelText='取消'
          >
            <div>
              <span>回退理由 ：</span><Input onChange={this.rollBackReason} size='small' style={{width:'200px'}} width={100} />
              <div style={{marginTop:'12px'}}>是否确定回退到当前节点？</div>
            </div>
          </Modal>
          <div className={ "drawContainer" } style={{position:'relative',marginLeft: "70px"}} id={containerId + containerIdPreFix}>
            <div className="ant-tooltip  ant-tooltip-placement-top" style={{
              position:'absolute',
              left:`${this.state.rulesPositionX-150}px`,
              top:`${this.state.rulesPositionY-90}px`,
              zIndex:9999,
              // padding:'6px 10px',
              // background:'rgba(0,0,0,0.7)',
              // borderRadius:'4px',
              // color:'#fff',
              width:`${this.state.ruleModalWidth}px`,
              display:this.state.expressionVisible===-1||this.state.expressionVisible===undefined?'none':'block'
            }}>
              <div className="ant-tooltip-content">
                <div className="ant-tooltip-arrow"></div>
                <div className="ant-tooltip-inner">
                  {this.state.expressionVisible}
                </div>
              </div>
            </div>
            <div style={{ fontSize: "18px", marginTop: "20px", marginLeft: "50px" }}>{ this.props.title }</div>
            {(this.props.sign&&this.props.sign==='claimTasks'?this.state.needRenderNodeList:this.state.nodeList)}
            {/*{this.state.needRenderNodeList}*/}
          </div>
        </div>
    )
  }
